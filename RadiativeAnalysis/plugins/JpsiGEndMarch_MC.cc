...
//object definition
class JpsiGEndMarch_MC : public edm::one::EDAnalyzer<> {
public:

  //constructor, function is called when new object is created
  explicit JpsiGEndMarch_MC(const edm::ParameterSet& conf);
...
JpsiGEndMarch_MC::JpsiGEndMarch_MC(const edm::ParameterSet& conf)
...
void JpsiGEndMarch_MC::endJob()
{
  ...
  cout << "HERE JpsiGEndMarch_MC::endJob()" << endl;
}

void JpsiGEndMarch_MC::analyze(
    const edm::Event& ev, const edm::EventSetup& es)
{
  std::cout << " -------------------------------- HERE JpsiGEndMarch_MC::analyze "<< std::endl;
  ...
  // output
  const double outArray[24] = {fittedDimuonVertexProb,fittedDimuonMass,maxMuonsVertexComp,lXY_fittedDimuon_bSpot,lXY_fittedDimuon_bSpot_significance,
  ...
  lifetimeBs,lifetimeModBs,muon1Id,muon2Id,(double)tight1,(double)tight2,triggerRes};
...
DEFINE_FWK_MODULE(JpsiGEndMarch_MC);