...
//
        ...
        for (const auto& conv : conversions) {
            ...
            //std::cout<<" The vertex fit flag is set to 1 for the converted photons : "<<dcv.vertexFitFlag<<"\n";
                ...
                MassLimits m_lim;
                rrt->FourvectorConvGammaPt_mmconvg_  = photon4vec.Pt();
                rrt->FourvectorConvGammaEta_mmconvg_ = photon4vec.Eta();
                rrt->FourvectorConvGammaPhi_mmconvg_ = photon4vec.Phi();
                ...
                rrt->FourvectorBsEta_mmconvg_  = BCand.Eta();
                ...
                rrt->BsCt3D_mmconvg_ = VariableDefinitions::calculateCt3D(PVvtxHightestPt, bVertex, bs);
                
                rrt->BsCt2D_mmconvg_ = VariableDefinitions::calculateCt2D(PVvtxHightestPt, bVertex, bs);
...
//////////////////////////////////////////////////////////////////
            ...
            rrt->FourvectorBsPt_mmrecog_   = BCand.Pt();
            rrt->FourvectorBsEta_mmrecog_  = BCand.Eta();
            rrt->FourvectorBsPhi_mmrecog_  = BCand.Phi();
            RefCountedKinematicParticle bs = BCandFitter.getBhardon();
            RefCountedKinematicVertex bVertex = BCandFitter.getVertex();
            AlgebraicVector7 b_par = bs->currentState().kinematicParameters().vector();
            GlobalVector Bsvec(b_par[3], b_par[4], b_par[5]);
            ...
            rrt->BsCt2D_mmrecog_ = VariableDefinitions::calculateCt2D(PVlowestDCA, bVertex, bs);

            rrt->BsCt3D_mmrecog_ = VariableDefinitions::calculateCt3D(PVlowestDCA, bVertex, bs);
            ...
            (Bsvec.x()*Bsvec.x()+Bsvec.y()*Bsvec.y());
                ...
                reco::Vertex PVvtxClosestZ = PVs[PVClosestZIndex]; 
                            ...
                            // PV variants with fitted momentum
                rrt->VertexfitBsCtErr2D_mmrecog_ = VariableDefinitions::calculateCt2DError(PVlowestDCA, bVertex, bs);
                
                rrt->VertexfitBsCtErr3D_mmrecog_ = VariableDefinitions::calculateCt3DError(PVlowestDCA, bVertex, bs);
                ...
                VertexDistance3D dist3D;
                rrt->VertexfitBsDist3D_mmrecog_ = dist3D.distance(PVlowestDCA, bVertex->vertexState()).value();
                rrt->VertexfitBsDist3DErr_mmrecog_ = dist3D.distance(PVlowestDCA, bVertex->vertexState()).error();

                VertexDistanceXY dist2D;
                rrt->VertexfitBsDist2D_mmrecog_ = dist2D.distance(PVlowestDCA, bVertex->vertexState()).value();
                rrt->VertexfitBsDist2DErr_mmrecog_ = dist2D.distance(PVlowestDCA, bVertex->vertexState()).error();
