// base from JpsiGStartApril_MC
// get dR(dimuon,photon), different matched muons
...
//object definition
class getDeltaR_MC : public edm::one::EDAnalyzer<> {
public:

  //constructor, function is called when new object is created
  explicit getDeltaR_MC(const edm::ParameterSet& conf);
...
getDeltaR_MC::getDeltaR_MC(const edm::ParameterSet& conf)
...
getDeltaR_MC::~getDeltaR_MC()
{
  cout <<" DTOR" << endl;
}

void getDeltaR_MC::beginJob()
{
  muonMVAIDProducer_ = new MuonMVAID(theConfig);

  tOut = new TNtupleD("tOut","Output tuple",
  "muonMatchedTwice:fittedDimuonVertexProb:fittedDimuonMass:dR_photonFittedDimuon:etaPhoton:muon1Id:muon2Id:dEta:cosdPhi:pTdimuon:pTphoton:etaDimuon:triggerRes");
  cout << "HERE getDeltaR_MC::beginJob()" << endl;
}

void getDeltaR_MC::endJob()
{
  ...
  cout << "HERE getDeltaR_MC::endJob()" << endl;
}

void getDeltaR_MC::analyze(
    const edm::Event& ev, const edm::EventSetup& es)
{
  std::cout << " -------------------------------- HERE getDeltaR_MC::analyze "<< std::endl;
  ...
  // check if one reco muon matched twice
  bool muonMatchedTwice = (reco::deltaR(*recoMatchedMuons.at(0), *recoMatchedMuons.at(1)) == 0.0);
  ...
  tOut->Fill((double)muonMatchedTwice,fittedDimuonVertexProb,fittedDimuonMass,deltaR_photon_fittedDimuon,eta,muon1Id,muon2Id,
    deltaEta_photon_fittedDimuon,cosDeltaPhi_photon_fittedDimuon,fittedDimuonLV.Pt(),photonLV.Pt(),fittedDimuonLV.Eta(),triggerRes);
...
DEFINE_FWK_MODULE(getDeltaR_MC);