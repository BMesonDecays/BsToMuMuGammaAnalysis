    ...
    RadiativeRootTree* rrt){
      ...
      for(size_t i =0 ; i<tracks.size(); i++){
          ...
          for(size_t j = i+1 ; j<tracks.size(); j++){
              ...
              rrt->BsVtxProb_mmkk_ = BCandFitter.getBhadronProb();
                // Bs kinematics from the vertex fit
                rrt->VertexfitBsMass_mmkk_ = BCandFitter.getBhadronMass();
                rrt->VertexfitBsPt_mmkk_   = Bsvec.perp();
                rrt->VertexfitBsEta_mmkk_  = Bsvec.eta();
                rrt->VertexfitBsPhi_mmkk_  = Bsvec.phi();
                rrt->VertexfitBsPz_mmkk_  = Bsvec.z();

                // secondary vertex position from the vertex fit
                rrt->VertexfitBsSVx_mmkk_ = bVertex->position().x();
                rrt->VertexfitBsSVy_mmkk_ = bVertex->position().y();
                rrt->VertexfitBsSVz_mmkk_ = bVertex->position().z();
                ...
                rrt->BsCt3D_mmkk_ = calculateCt3D(PVlowestDCA, bVertex, bs);

                rrt->VertexfitBsCtErr3D_mmkk_ = calculateCt3DError(PVlowestDCA, bVertex, bs);
                
                rrt->BsCt2D_mmkk_ = calculateCt2D(PVlowestDCA, bVertex, bs);

                rrt->VertexfitBsCtErr2D_mmkk_ = calculateCt2DError(PVlowestDCA, bVertex, bs);
...
double ReferenceModeratorVertex::calculateCt2D(const reco::Vertex& PV,
...
double ReferenceModeratorVertex::calculateCt3D(const reco::Vertex& PV,
                                     const RefCountedKinematicVertex& bVertex,
                                     const RefCountedKinematicParticle& bCand)
{
    MassLimits m_lim;
...
double ReferenceModeratorVertex::calculateCt2DError(const reco::Vertex& PV,
							                 const RefCountedKinematicVertex& bVertex,
							                 const RefCountedKinematicParticle& bCand)
{
    MassLimits m_lim;
...
double ReferenceModeratorVertex::calculateCt3DError(const reco::Vertex& PV,
                                             const RefCountedKinematicVertex& bVertex,
                                             const RefCountedKinematicParticle& bCand)
{
    MassLimits m_lim;
