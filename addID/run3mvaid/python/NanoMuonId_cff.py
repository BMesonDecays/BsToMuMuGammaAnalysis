...
for entry in xgboost_models:
    NanoMuonId.xgboost_models.append(entry[0]),
    NanoMuonId.xgboost_variable_names.append(entry[1])

from Configuration.Eras.Modifier_run2_muon_2016_cff import run2_muon_2016
run2_muon_2016.toModify(NanoMuonId,
    triggerCollection = cms.string("hltL3MuonCandidates")
)

NanoMuonIdMc = NanoMuonId.clone( isMC = cms.bool(True) ) 

NanoMuonIdVariables = cms.PSet(
...
for trigger in triggers:
    setattr(NanoMuonIdVariables, trigger, Var("userInt('%s')" % trigger, int, doc = "Used in trigger"))

for entry in xgboost_models:
    setattr(NanoMuonIdVariables,
            "xgb_%s" % entry[1],
            Var("userFloat('xgb_%s')" % entry[1], float, doc = "New XGBoost MVA id")
            )

NanoMuonIdMcVariables = merge_psets(
    NanoMuonIdVariables,
    cms.PSet(
...
NanoMuonIdTable=cms.EDProducer("SimpleCompositeCandidateFlatTableProducer", 
    src=cms.InputTag("NanoMuonId","muons"),
    cut=cms.string(""),
    name=cms.string("MuonId"),
    doc=cms.string("Muon Id Variables"),
    singleton=cms.bool(False),
    extension=cms.bool(False),
    variables = NanoMuonIdVariables
)
NanoMuonIdMcTable=cms.EDProducer("SimpleCompositeCandidateFlatTableProducer", 
    src=cms.InputTag("NanoMuonIdMc","muons"),
    cut=cms.string(""),
    name=cms.string("MuonId"),
    doc=cms.string("Muon Id Variables"),
    singleton=cms.bool(False),
    extension=cms.bool(False),
    variables = NanoMuonIdMcVariables
)

NanoMuonIdSequence   = cms.Sequence(NanoMuonId)
NanoMuonIdMcSequence = cms.Sequence(NanoMuonIdMc)
NanoMuonIdTables     = cms.Sequence(NanoMuonIdTable)
NanoMuonIdMcTables   = cms.Sequence(NanoMuonIdMcTable)
